...
export class LoginPage {
    ...
    forgotPwdSubmit() {
        ...
        loader.present().then(() => {
            this.loginApi.forgotPwd(this.userObj.email).subscribe(
                response => {
                    // this.authRes = response.erroMsg;
                    // if (!response.erroMsg) {
                    //     this.validUser(response);
                    ...
                    this.forgotPwdFailed = !this.forgotPwdSuccess;
                    loader.dismiss();
                },
                error => {
    ...
    gotoChangePwd() {
        this.userInfoService.setUserInfo({ email: this.userObj.email });
